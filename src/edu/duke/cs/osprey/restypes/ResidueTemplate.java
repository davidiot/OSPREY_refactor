...
public class ResidueTemplate implements Serializable {
	...
	// We need to identify the type of each residue whenever we want to move it
	// it a residue-specific way
	// (e.g. to define sidechain dihedrals and rotamers),
	// or whenever we want to evaluate its energy (except by an ab initio method
	// that takes only element
	...
	// "standard" residue
	// info from here will mostly be copied into any residue in the structure to
	...
	public int dihedral4Atoms[][];// for each dihedral, list of 4 atoms defining
									// it
	// these are indices in all our atom-wise arrays
	public ArrayList<ArrayList<Integer>> dihedralMovingAtoms;// list of atoms
																// that move for
																// each dihedral

	// Rotameric information for this residue template. ResidueTemplate supports
	// both backbone dependent and backbone independent rotamer libraries.
	// PGC 2015: If backbone dependent rotamer libraries are used, the number of
	// rotamers is dependent on the backbone dihedrals.
	...
	// note: this is just the standard set of ideal rotamers, we can modify this
	// as desired
	double rotamericDihedrals[][][][];// (# phi increments)x(# psi increments) x
										// numRotamers x numDihedrals array.
	// Number of "bins" for Phi and Psi angles for backbone dependent rotamer
	// libraries. For example, in the Dunbrack library there are 37 bins for
	// each dihedral.
	// We assume that the number of bins will always be the same for phi or psi.
	public int numberOfPhiPsiBins = -1;
	// Resolution of phi and psi backbone dihedrals in the backbone dependent
	// rotamer library. For example, in the Dunbrack rotamer library the
	// resolution is 10.
	public double phiPsiResolution = -1;

	public ResidueTemplate(Residue res, String name) {
		// initializes only with info from a template file. res won't even have
		// coordinates yet
		templateRes = res;
		this.name = name;
	}
	...
	public int[] getDihedralDefiningAtoms(int dihedralNum) {
		// numbers (indices among the atoms in this template) of the 4 atoms
		// defining the dihedral
		return dihedral4Atoms[dihedralNum];
	}

	public ArrayList<Integer> getDihedralRotatedAtoms(int dihedralNum) {
		// the atoms that actually move (including the 4th of the
		// dihedral-defining atoms)
	...
	void computeDihedralMovingAtoms() {
		...
		// we assume atom 4 (from the dihedral4Atoms) moves and the others are
		// fixed;
		// thus the moving atoms consist of the atoms bonded to atom 3
		// (excluding atom 2)
		// and anything beyond them (compute recursively). If any of these
		// moving atoms bond back to atom 2
		...
		for (int dihedNum = 0; dihedNum < numDihedrals; dihedNum++) {
			...
			ArrayList<Integer> movingAtoms = new ArrayList<>();// indices of
																// atoms that
																// move (in
																// templateRes.atoms)
	...
	private void movingAtomsDFS(Atom atom, ArrayList<Integer> movingAtoms, int[] dihedAtoms) {
		...
		if (atom == null) // atom is in another residue...this is an error
			throw new RuntimeException(
					"ERROR: Trying to define non-free dihedral in rotamer library (bonds to another residue)");

		int atomIndex = templateRes.getAtomIndexByName(atom.name);

		if (atomIndex == dihedAtoms[0] || atomIndex == dihedAtoms[1]) // there
																		// is a
																		// cycle
																		// within
																		// the
																		// residue
			throw new RuntimeException(
					"ERROR: Trying to define non-free dihedral in rotamer library (dihedral is in a ring)");

		if (atomIndex == dihedAtoms[2]) // dihedAtoms[2] doesn't move, though it
										// is bonded to moving atoms
			return;

		// check that we haven't already counted atom as moving (since cycles
		// are allowed within the set of moving atoms)
	 ...
	 * @param rotNum
	 *            The rotamer's index number
	 * @param dihedralNum
	 *            The dihedral's index number.
	...
	public double getRotamericDihedrals(double phi, double psi, int rotNum, int dihedralNum) {
		...
		if (Double.isNaN(phi) || Double.isNaN(psi)) {// dihedrals not defined
														// for this residue
		...
		// Under the dunbrack rotamer library, backbone dependent rotamers have
		// a resolution of 10 degrees,
		// while in backbone-independent rotamer libraries they have a
		// resolution of 360.
		// Therefore, we round to the closest "bin" and add numberOfPhiPsiBins/2
	 ...
	 * 
	 * @param phi
	 *            The backbone phi angle, ignored if backbone independent
	 *            rotamer library.
	 * @param psi
	 *            The backbone psi angle, ignored if backbone independent
	 *            rotamer library.
	...
	/**
	 * Resolution of backbone "bins" in the rotamer library. For example in as
	 ...
	 *            Number of rotamers
	 * @param phiBin
	 *            Bin in the rotamer array where the rotameric dihedrals will be
	 *            stored.
	 * @param psiBin
	 *            Bin in the rotamer array where the rotameric dihedrals will be
	 *            stored.
