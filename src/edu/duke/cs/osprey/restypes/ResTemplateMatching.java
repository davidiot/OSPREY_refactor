...
public class ResTemplateMatching {
	...
	int[] matching;// for each atom in the template, index of the corresponding
					// atom in the residue

	int[] partialMatching;// We'll use this during our depth-first search for
							// matchings;
	// any good ones will be copied to matching

	public double score;// least-square deviations in bond lengths, for atoms
						// pairs bonded in the template
	// if all atoms bonded in the template are bonded, then we expect this is
	// the right template and matching

	int numAtoms;// number of atoms in the residue (should be the same as in the
					// template)

	// some things useful in performing the assignment
	ArrayList<ArrayList<Integer>> templateBonds;// For each template atom, list
												// of indices of other atoms
	// in the template that it's bonded to
	double[][] residueDistanceMatrix;// matrix of distances between all atoms in
										// the non-template residue
	...
	public ResTemplateMatching(Residue res, ResidueTemplate template) {
		...
		score = Double.POSITIVE_INFINITY;// start without any good matching
											// found

		ArrayList<Atom> templateAtoms = template.templateRes.atoms;
		numAtoms = templateAtoms.size();

		if (res.atoms.size() != numAtoms) {// matching impossible: not even
											// number of atoms matches
			return;
		}
		...
		if (template.templateRes.coords != null) // we have coordinates for
													// templateRes...can use
													// real coordinates
			templateDistanceMatrix = template.templateRes.atomDistanceMatrix();
		else// we actually only need estimates of bond lengths in the
			// template...other distances don't matter
		...
		// if this default-order depth-first search is too slow, maybe try
		// searching
	...
	void searchForMatching(int level) {
		// DFS over permutations of atoms, starting at current level (indicates
		// which atom
		// in the template we want to match next)

		if (level == numAtoms) {// all atoms defined...let's score this matching
			// when we get a full permutation, we score it based on least-square
			// length difference for the template bonds.
			double curMatchingScore = scoreCurPartialMatching();
			if (curMatchingScore < score) {// best so far
				score = curMatchingScore;
				System.arraycopy(partialMatching, 0, matching, 0, numAtoms);// store
																			// the
																			// matching
																			// as
																			// well
			...
			for (int resAtNum = 0; resAtNum < numAtoms; resAtNum++) {
				if (!atomAlreadyInPartialMatching(resAtNum, level)) {
					...
					if (templateEleType.equalsIgnoreCase(resEleType)) {
						...
						if (!canPrunePartialMatching(level)) // we prune
																// branches if
																// atom pairs
																// that are
																// supposed to
	...
	boolean atomAlreadyInPartialMatching(int resAtNum, int level) {
		// indicates that atom can't be put into partialMatching (currently
		// filling in given level)
	...
	boolean canPrunePartialMatching(int level) {
		...
		for (int bondedTemplateAtNum : templateBonds.get(level)) {
			if (bondedTemplateAtNum < level) {// and thus bondedTemplateAtom is
												// already assigned in matching
				...
				if (resDist >= 1.5 * templateDist) // over 1.5 x template
													// length--not a feasible
													// bond
	...
	double scoreCurPartialMatching() {
		...
		for (int templateAtNum = 0; templateAtNum < numAtoms; templateAtNum++) {
			for (int bondedTemplateAtNum : templateBonds.get(templateAtNum)) {
				// we need to compare the templateAtNum-bondedTemplateAtom
				// distance
		...
		// This way we don't have to make assumptions about max bond length
		// (Except for liberal 2x)
