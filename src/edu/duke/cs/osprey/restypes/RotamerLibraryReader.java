...
public class RotamerLibraryReader implements Serializable {
	...
	public static void readRotLibrary(String rotFilename, ResidueTemplateLibrary templateLib) {
		...
		try {
			...
			curLine = bufread.readLine();// skip over number of residues in
											// library
			...
			while (curLine != null) {
				...
				String dihedralAtomNames[][] = new String[numDihedrals][4];// atoms
																			// involved
																			// in
																			// each
																			// dihedral
				double rotamerValues[][] = new double[numRotamers][numDihedrals];// dihedral
																					// values
																					// at
																					// each
																					// rotamer
				...
				// we've now got all the rotamer information for the residue
				// type aaName
				...
				for (ResidueTemplate template : templateLib.templates) {
					if (template.name.equalsIgnoreCase(aaName)) {
						...
						// Backbone independent rotamer libraries have a single
						// "bin" for backbone dihedrals.
						template.setNumberOfPhiPsiBins(1);
						// And the resolution for a backbone independent rotamer
						// library's bin is 360 (i.e. there is a single bin)
						template.setRLphiPsiResolution(360);
						template.initializeRotamerArrays();

						// Use 0 and 0 for the phi and psi angles, to indicate a
						...
						// convert atom names to indices for the atoms in
						// templateRes
						...
						for (int dihedNum = 0; dihedNum < numDihedrals; dihedNum++) {
							for (int a = 0; a < 4; a++) {
								...
								template.dihedral4Atoms[dihedNum][a] = template.templateRes
				...
				if (!foundTemplate) {
					throw new RuntimeException("ERROR: Have rotamer information for residue type " + aaName
							+ " but can't find a template for it");
				}
	 ...
	 *         For Amino Acid Type given a Phi and Psi angle value. This class
	 *         is a data structure that stores all the rotamers for a specific
	 *         phi and psi bin, given an angle.
	 */
	 ...
	 * @author PGC 2015
	 * 
	 *         BBDepRotamersForType: Backbone dependent rotamers for type. This
	 *         class is a data structure that stores all the backbone rotamers
	 *         for a specific amino acid type. It is used by the Dunbrack
	 *         parser.
	 */
	private static class BBDepRotamersForType {
		...
		// resolution: The increments on which the rotamer library classifies
		// rotamers. For Dunbrack this is 10.
		public double resolution;
		// numBins: The number of "bins" for each phi and psi. For Dunbrack this
		// is 37 (e.g. at -180, -170, ..., 0, ..., 170, 180)
		public int numBins;
		// The array structure that stores the rotamers for each combination of
		// phi and psi angles. The size of this 2D array is numBins*numBins.
		public BBDepRotamersForTypePhiAndPsi rotamersForType[][];

		/**
		 * 
		 * @param aaType
		 *            amino acid type for which we are storing the rotamers.
		 * @param numBins
		 *            The increments on which the rotamer library classifies
		 *            rotamers. For Dunbrack this is 10.
		 * @param resolution
		 *            The number of "bins" for each phi and psi. For Dunbrack
		 *            this is 37 (e.g. at -180, -170, ..., 0, ..., 170, 180)
		 */
		public BBDepRotamersForType(String aaType, int numBins, double resolution) {
			...
			for (int phiBin = 0; phiBin < numBins; phiBin++) {
				...
				// Each bin corresponds to a phi and psi value. We can compute
				// each using the resolution and numBins info.
				double phi = (phiBin - numBins / 2) * resolution;
				for (int psiBin = 0; psiBin < numBins; psiBin++) {
					double psi = (psiBin - numBins / 2) * resolution;
					// Each field in rotamersForType stores a data structure
					// with all the rotamers for that phi and psi bin.
		 ...
		 * 
		 * @param phi
		 *            The phi angle for this rotamer
		 * @param psi
		 *            The psi angle for this rotamer.
		 * @param dihedralValues
		 */
	 ...
	 * 
	 * @param templateLib
	 *            The library of residue templates.
	 */
	public static void readDunbrackRotamerLibraryForResiduePosition(String rotFilename,
		...
		int binsDunbrack = 37;
		// Create a hash map to map every amino acid in the template library to
		// its rotamers
		HashMap<String, BBDepRotamersForType> allDunbrackRotamersMap = new HashMap<String, BBDepRotamersForType>();
		for (ResidueTemplate template : templateLib.templates) {
			BBDepRotamersForType bbDepRotamerForTypeOfTemplate = new RotamerLibraryReader.BBDepRotamersForType(
					template.name, binsDunbrack, dunbrackResolution);
			allDunbrackRotamersMap.put(template.name, bbDepRotamerForTypeOfTemplate);

		}
		String curLine;
		// Read the file name for the Dunbrack Backbone dependent rotamer
		// library.
		try {
			...
			BufferedReader bufread = new BufferedReader(new InputStreamReader(is));
			curLine = null;
			// Parse the whole file first and add each rotamer entry to the
			// array list
			while ((curLine = bufread.readLine()) != null) {
				...
				if (!curLine.startsWith("#")) {
					...
					// We compute the number of dihedrals for this AA by
					// counting the number of none zeroes in columns r1-r4.
					...
					// Parse the rotamer probability which is located at
					// position 37 to 44.
					String probabilityStr = "" + dbLine[37] + dbLine[38] + dbLine[39] + dbLine[40] + dbLine[41]
							+ dbLine[42] + dbLine[43] + dbLine[44];
					double myProbability = Double.parseDouble(probabilityStr.trim());

					// The first char for each of the dihedrals is, respectively
					// at 47, 55, 63, 71
					...
					for (int chiIx = 0; chiIx < numDih; chiIx++) {
						String dihedralStr = "" + dbLine[dihedralIndices[chiIx]] + dbLine[dihedralIndices[chiIx] + 1]
								+ dbLine[dihedralIndices[chiIx] + 2] + dbLine[dihedralIndices[chiIx] + 3]
								+ dbLine[dihedralIndices[chiIx] + 4] + dbLine[dihedralIndices[chiIx] + 5];
					...
					if (myProbability >= EnvironmentVars.DUNBRACK_PROBABILTY_CUTOFF) {
						...
						// The dunbrack rotamer library contains some rotamers
						// not defined by our template.
		...
		for (ResidueTemplate myTemplate : templateLib.templates) {
			...
			for (int phiBin = 0; phiBin < binsDunbrack; phiBin++) {
				for (int psiBin = 0; psiBin < binsDunbrack; psiBin++) {
					double rotamers[][] = (allDunbrackRotamersMap.get(myTemplate.name)).rotamersForType[phiBin][psiBin]
							.convertRotamersToArray();
	...
	/*
	 * //reads in the rotamer volume data from volFilename public void
	 * loadVolFile (String volFilename){
	 * 
	 * try { readRotVol(volFilename); } catch (Exception e){ System.out.println(
	 * "Rotamer volumes file not found. Computing it..");
	 * computeAAVolumes(volFilename); try { readRotVol(volFilename); } catch
	 * (Exception e1){ System.out.println("ERROR: "+e1); System.exit(1); } } }
	 * 
	 * //Read in all of the rotamer volumes for all amino acids from the
	 * volFilename file private void readRotVol(String volFilename) throws
	 * Exception {
	 * 
	 * FileInputStream is = new FileInputStream( volFilename ); BufferedReader
	 * bufread = new BufferedReader(new InputStreamReader(is)); String curLine =
	 * null;
	 * 
	 * rotamerVolumes = new double[numAAallowed][];
	 * 
	 * is = new FileInputStream( volFilename ); bufread = new BufferedReader(new
	 * InputStreamReader(is));
	 * 
	 * // Skip over comments (lines starting with !) curLine =
	 * bufread.readLine(); while( curLine.charAt(0) == '!' ) curLine =
	 * bufread.readLine();
	 * 
	 * int curResult = 0; while( curLine != null ) {
	 * 
	 * int aaIndex = getAARotamerIndex(StringParsing.getToken(curLine,1));
	 * 
	 * int numRotVol = numRotamers[aaIndex]; if (numRotamers[aaIndex]==0)
	 * numRotVol++;
	 * 
	 * rotamerVolumes[aaIndex] = new double[numRotVol]; for (int j=0;
	 * j<numRotVol; j++) rotamerVolumes[aaIndex][j] = new
	 * Double(StringParsing.getToken(curLine,j+2)).doubleValue();
	 * 
	 * curLine = bufread.readLine(); curResult++; }
	 * 
	 * bufread.close();
	 * 
	 * if (curResult!=numAAallowed){ System.out.println(
	 * "ERROR: not all amino acid types read from rotamer volumes file");
	 * System.exit(1); } }
	 * 
	 * // Uses the VolModule class to calculate the volume of each rotamer of
	 * each amino acid public void computeAAVolumes(String volFileName) {
	 * 
	 ...
	 * 
	 * Residue res = ppr.getResidue("ala"); //res.fullName = "ALA  ";
	 * m.addResidue(0,res);
	 * 
	 * VolModule sm = new VolModule(m); sm.setResidueTreatment(0,1);
	 * 
	 * LR = new StrandRotamers(rotFile,m.strand[0]);
	 * 
	 * PrintStream printStream = setupOutputFile(volFileName);
	 * 
	 * String aanames[] = getAAtypesAllowed(); int numAAs = getNumAAallowed();
	 * 
	 * for(int i=0;i<numAAs;i++){ if(canMutate)
	 * LR.changeResidueType(m,0,aanames[i],true); printStream.print(aanames[i] +
	 * " "); System.out.println(aanames[i] + " ");
	 * if(getNumRotamers(aanames[i])==0){ // ALA or GLY double vol =
	 * sm.getMoleculeVolume(0.25f,0.0f); printStream.print(vol + " ");
	 * System.out.println(vol + " "); } for(int
	 * j=0;j<getNumRotamers(aanames[i]);j++){ LR.applyRotamer(m,0,j); double vol
	 * = sm.getMoleculeVolume(0.25f,0.0f); printStream.print(vol + " ");
	 * System.out.println(vol + " "); } printStream.println(); }
	 * printStream.close(); }
	 */
