...
/**
 * 
 * Description of the perturbations in our system, not set up in a molecule
 * (describes the content of a .pert file)
 * 
 * @author mhall44
 */
public class PertSet {

	ArrayList<String> pertTypes = new ArrayList<>();// List of types ("BACKRUB",
													// etc.) for the
													// perturbations
	ArrayList<ArrayList<String>> resNums = new ArrayList<>();// corresponding
																// perturbations'
	// lists of directly-affected residue numbers (PDB numbering)

	ArrayList<ArrayList<double[]>> pertIntervals = new ArrayList<>();
	// for each of the perturbations, a list of intervals for its parameter to
	// be in
	// these will be used to define RCs

	ArrayList<ArrayList<ArrayList<int[]>>> pertStates = new ArrayList<>();
	// for each flexible residue position, for each "perturbation state" of the
	// residue,
	// a list of pairs (pert#, interval#) defining what perturbations (indexed
	// in pertNames)
	...
	// For each perturbation, the lines of additional info are listed here (null
	// if none)
	...
	public boolean loadPertFile(String pertFileName, boolean loadStates) {
		...
		// Load perturbations and their intervals; if loadStates then residue
		// pert states too
		try {
			...
			if (loadStates) {
				...
				while (br.readLine() != null) {// Read residue perturbation
												// states. Skipping the "RES"
												// line
					...
					// indices of perturbations that can change the conf of this
					// residue
					// (to be listed presently)

					// Read the perturbations affecting this residue
					// This section is included in the file because it specifies
					// if some indirect effects should be neglected
					...
					for (int a = 0; a < numStates; a++) {// Read perturbation
															// states
						ArrayList<int[]> pertState = new ArrayList<>();

						st = new StringTokenizer(br.readLine(), " ");
						for (int b = 0; b < resNumPerts; b++) {
							int[] p = new int[] { resPerts.get(b), Integer.valueOf(st.nextToken()) };// pert
																										// #,
																										// interval
																										// #
							pertState.add(p);
			...
			br.close();
		} catch (FileNotFoundException e) {// basically means we should select
											// perturbations
			return false;
	...
	void recordAdditionalInfo(BufferedReader br, String pertType) throws IOException {
		...
		if (pertType.equalsIgnoreCase("PARTIAL STRUCTURE SWITCH")) {
			...
			// 2 structures
			// 1ABC.pdb
			// (JUST ONE PDB LISTED, BECAUSE THE FIRST STRUCTURE IS ALWAYS THE
			// ORIGINAL ONE)
	...
	public void writePertFile(String pertFileName) {
		try {
			...
			for (int pertNum = 0; pertNum < numPerts; pertNum++) {// Perturbation
																	// info
			...
			for (int pos = 0; pos < pertStates.size(); pos++) {// Residue
																// perturbation
																// state, RC
																// info
				...
				// UNLIKE OSPREY 2.X WILL HAVE RES PERT STATES RECORD FOR EVERY
				// RESIDUE
				...
				if (resPertStates.size() > 0) {
					...
					for (int a = 0; a < firstState.size(); a++)
						bw.append(firstState.get(a)[0] + " ");// firstState
																// consists of
																// pairs (pert
																// #, interval
																// #)
	...
	private void writeAdditionalInfo(BufferedWriter bw, ArrayList<String> altPDBs) throws IOException {
		...
		if (altPDBs != null) {// there is some info needed (only for partial
								// structure switch currently)
