...
/**
 *
 * Records backbone coordinates for a residue, so we can restore them when
 * performing perturbations Sidechain is moved as a rigid body (including CA)
 * 
 * DZ: Adding RNA BB states to this.
 * 
 * @author mhall44
 */
public class ResBBState implements Serializable {
	...
	public ResBBState(Residue res) {
		...
		// Thus, we want to mark NA residues first.
		type = BBType.AA;
	...
	public void putInState(Residue res) {
		// Put res in the conformational state defined by the backbone
		// coordinates recorded here

		Set<String> BBAtomNames = coords.keySet();

		// If there is a CB, move the sidechain and HA as a rigid body including
		// CA
		// gly HA's can be placed exactly by sidechain idealization, so don't
		// worry about them
		if (CBCoord != null) {
		...
		for (String atomName : BBAtomNames) {
			...
			if (atomIndex == -1) {
				if (res.fullName.startsWith("PRO") && atomName.equalsIgnoreCase("H")) {// mutating
																						// to
																						// PRO...use
																						// H
																						// for
					...
					atomCoords = atomCoords.clone();// will modify to make CD
													// coords
