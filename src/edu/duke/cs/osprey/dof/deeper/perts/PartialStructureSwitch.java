...
/**
 *
 * This is a perturbation that substitutes in the backbone conformation from
	...
	public boolean doPerturbationMotion(double paramVal) {
		...
		if (structNum > 0) {// 0 indicates a state besides the original one
			for (int resNum = 0; resNum < resDirectlyAffected.size(); resNum++) {
				 ...
				 * 
				 * for(int atomNum=0; atomNum<numAtoms; atomNum++){ String
				 * atomName = res.atoms.get(atomNum).name;
				 * if(isBBAtom(res,atomName)){//BB atom: copy coordinates int
				 * atomNumAlt = altConf.getAtomIndexByName(atomName);
				 * System.arraycopy(altConf.coords, 3*atomNumAlt, res.coords,
				 * 3*atomNum, 3); } }
				 */
			}
		}

		return true;// A partial structure switch will always put in structure
					// as given.
	}
}
