...
public class Backrub extends Perturbation {
	...
	public RigidBodyMotion[] calcTransRot(double param) {
		...
		double[] oldO1Coord = extractA();
		double[] oldO2Coord = extractB();

		double rotax[] = VectorAlgebra.subtract(x[2], x[0]);// vector from 1st
															// to second Calpha:
															// primary rotation
															// axis

		// Create the primary rotation matrix. This can be used to rotate about
		// either anchor CA.
	...
	// DZ: These next three methods extract the atoms we need. Refactored out to
	// prevent duplicate code in RNArubs
	public double[][] extractX() {
		// C-alphas in a backrub
	...
	public double[] extractA() {
		return resDirectlyAffected.get(0).getCoordsByAtomName("O");
	}

	public double[] extractB() {
		return resDirectlyAffected.get(1).getCoordsByAtomName("O");
	}

	// Get the small rotation angle that will rotate atom pp1 around the axis
	// defined by atoms (pp2,a3), so that pp1 will be as close as possible to
	// atom a4
	// Adapted from Backrubs to use only Atom.coord arrays (needed for the setup
	...
	// The next three function are from Backrubs; they now operate on
	// coordinates (double[])
	...
	public double[] projectPointLine(double[] l1, double[] l2, double[] p3) {
		...
		double u = ((p3[0] - l1[0]) * (l2[0] - l1[0]) + (p3[1] - l1[1]) * (l2[1] - l1[1])
				+ (p3[2] - l1[2]) * (l2[2] - l1[2]));
		u = u / d12sq;
