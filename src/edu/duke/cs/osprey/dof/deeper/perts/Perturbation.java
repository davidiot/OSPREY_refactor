 ...
 * 
 * Note: This DEEPer package, unlike the rest of the software, consists of
 * perturbations specific to proteins and so it will make explicit reference to
 * protein atoms outside HardCodedResidueInfo DEEPer perturbations can be used
 * in the protein part of a mixed protein-nonprotein system
 * 
 * DZ: adding RNA perturbations
 * 
 * @author mhall44
 */
public abstract class Perturbation extends DegreeOfFreedom {
	...
	public void apply(double paramVal) {
		...
		// OK now we will revert all successors to the state they were at before
		// this perturbation
		// and any successors were applied
		// The backbone is reverted exactly, and the sidechain follows as a
		// rigid body
		ArrayList<Double> dependentGenChi1 = new ArrayList<>();
		for (Residue res : dependentResidues) {
			dependentGenChi1.add(GenChi1Calc.getGenChi1(res));// record gen chi1
																// so we can
																// restore it
																// later
		...
		// OK now we can actually apply the perturbation motion, and update
		// pre-pert states for any successors
		doPerturbationMotion(paramVal);
		block.updateSuccessorPrePertStates(indexInBlock);

		// OK we now have to restore the correct parameter values for each of
		// the successors,
		// and if they have successors the pre-pert states need to be updated
		// too
		...
		// We now place the sidechains (by idealization, and appropriate setting
		// of gen chi1)
		// other aspects of sidechain geometry (chi2, etc.) will be correct
		// because the sidechain
		// is treated as a rigid body (except for Pro, which has no other
		// sidechain DOFs)
	...
	// actually handle the perturbation motion, moving the backbone atoms to the
	// desired position
	...
	protected void movePeptidePlane(RigidBodyMotion motion, int startingRes, boolean includeFinalSCH) {
		// Apply motion to the peptide plane between
		// resDirectlyAffected[startingRes] and
		// resDirectlyAffected[startingRes+1]
		...
		if (includeFinalSCH) {
			...
			for (int atomIndex = 0; atomIndex < secondRes.atoms.size(); atomIndex++) {
				...
				if (!(atomName.equalsIgnoreCase("C") || atomName.equalsIgnoreCase("O")
			...
			ArrayList<String> res2PepAtoms = new ArrayList<>();// list of atoms
																// in 2nd res to
																// move
			res2PepAtoms.add("N");

			// secondRes will have either an amide H, or be Pro (CD takes the
			// place of H)
	...
	protected void applyBackrubLikeMotion(RigidBodyMotion[] pepRots) {
		// The backrub motion acts on the backbone with two peptide-plane
		// rotations
		// (both already composed with a primary rotation about the CA0-CA2
		// axis)
		// This motion can be used to apply both the Backrub and
