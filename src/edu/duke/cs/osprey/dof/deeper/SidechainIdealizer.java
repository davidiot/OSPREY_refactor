...
/**
 *
 * This class handles idealization of sidechains; residues moved by DEEPer have
 * idealized sidechains because orig sidechain geometry not valid for perturbed
 * backbones
 * 
 * @author mhall44
 */
public class SidechainIdealizer {
	...
	// This function is based on chiropraxis.sc.SidechainIdealizer.idealizeCB
	// from KiNG
	// The whole sidechain idealization (SidechainIdealizer.idealizeSidechain)
	// is not performed because
	// a mutated residue would already be in an idealized conformation, and if
	// the residue is not already
	...
	public static void idealizeSidechain(Residue res) {
		...
		if (res.template.name.equalsIgnoreCase("GLY")) {
			...
			int HASystem = 0;// The system used to name HA's. 0=HA2/HA3,
								// 1=1HA/2HA, 2=2HA/3HA
			...
			System.arraycopy(newHA, 0, res.coords, HANum * 3, 3);// Change the
																	// HA
																	// coordinates
			...
			} else if (res.template.name.equalsIgnoreCase("VAL") || res.template.name.equalsIgnoreCase("THR")
					|| res.template.name.equalsIgnoreCase("ILE")) {
			...
			// Now the rotation matrix for the sidechain will give the smallest
			// rotation
			...
			if (VectorAlgebra.norm(rotax) != 0) {// CB not already in the ideal
													// position
			...
			if (HardCodedResidueInfo.hasNucleicAcidBB(res)){
				HANum = res.getAtomIndexByName("H1'");
			}
			System.arraycopy(newHA, 0, res.coords, HANum * 3, 3);// Change the
																	// HA
																	// coordinates
		...
		if (res.template.name.equalsIgnoreCase("PRO")) // Idealize the CG, CD
														// and ring hydrogen
														// positions
			idealizeProRing(res);
		// For proline, the above sidechain idealization operations do not move
		// CD
	}
	...
	static boolean isSidechainAtom(String atomName, Residue res) {
		// check by name if the atom is in the sidechain (i.e., the atom from CB
		// onward,
	...
	public static boolean idealizeProRing(Residue res) {
		// Make an idealized ring for the given residue, given the current
		// positions of the backbone atoms, CB, and CD
		// The pucker specified in res.pucker will be applied:
		// UP pucker means the smaller of the two possible solutions for chi1;
		// DOWN pucker means the larger.
		// The CA-N-CD and CA-CB-CG angles are adjusted to the ideal as they are
		// likely to be too large after mutations, etc.
		// Then chi1 is calculated, finding the two (UP and DOWN) solutions that
		// maintain the ideal CG-CD bond length
		...
		double canhat[] = VectorAlgebra.subtract(NCoord, CACoord);// Unit vector
																	// in CA -->
																	// N
																	// direction
		...
		System.arraycopy(CDCoord, 0, res.coords, 3 * CDNum, 3);// Store the new
																// CD
																// coordinates
		...
		cacbhat = VectorAlgebra.scale(cacbhat, 1 / VectorAlgebra.norm(cacbhat));// Unit
																				// vector
																				// in
																				// CA
																				// -->
		...
		System.arraycopy(CGCoord, 0, res.coords, 3 * CGNum, 3);// Store the new
																// CG
																// coordinates
																// (their
																// torsion will
		...
		// The position of CG will be given by CBCoord + dBG*(-cosa*cacbhat +
		// sina*(dir2*cos(chi1)+dir3*sin(chi1)))

		double CGCD = VectorAlgebra
				.norm(VectorAlgebra.subtract(idealPro.getCoordsByAtomName("CD"), idealPro.getCoordsByAtomName("CG")));
		...
		double A = ((r1 + cosa * dBG) * (r1 + cosa * dBG) + r2 * r2 + r3 * r3 + dBG * dBG * sina * sina - CGCD * CGCD)
				/ (2 * dBG * sina);
		...
		if (Math.abs(A / R) > 1) {
			...
			if (res.pucker.getCurPucker() == ProlinePucker.UP)
				...
				newChi1 = (double) Math.PI - beta - asr;// Larger newChi1, since
														// an arcsine is acute
														// (puckers merge when
														// asr = pi/2, i.e. A=R)
			...
			// This will mess up the CG-HG bonds but idealizeRingHydrogens will
			// fix that
	...
	public static void idealizeRingHydrogens(Residue res, Residue idealPro) {
		// Idealize the sidechain hydrogens (those bonded to CB, CG, and CD) of
		// a proline
		...
		// These values are from Allen et al, Chem. Eur. J. 2004, 10, 4512 –
		// 4517
		// which is an ab initio study of free neutral proline
		// They provide constraints for structural refinement given heavy-atom
		// positions,
		// including constant values for the above angles plus linear
		// combinations of angles like CA-CB-HB1
		// It is probably not worthwhile to include these small corrections at
		// runtime
		// (seems to require solving a trig equation that's an 8th-order
		// polynomial in half-angle tangent form,
		// plus the polypeptide chain geometry might change things)
		// So we assume C_2v symmetry (i.e. 2-fold rotational symmetry about
		// heavy-atom-angle bisector)
		// at CB, CG, and CD with the above angles

		double HBDist = VectorAlgebra
				.norm(VectorAlgebra.subtract(idealPro.getCoordsByAtomName("HB2"), idealPro.getCoordsByAtomName("CB")));// CB-HB2
																														// (or,
																														// assumed,
																														// CB-HB1
																														// distance)
																														...
																														// have
																														// it)
		double HGDist = VectorAlgebra
				.norm(VectorAlgebra.subtract(idealPro.getCoordsByAtomName("HG2"), idealPro.getCoordsByAtomName("CG")));
		double HDDist = VectorAlgebra
				.norm(VectorAlgebra.subtract(idealPro.getCoordsByAtomName("HD2"), idealPro.getCoordsByAtomName("CD")));
		...
		double BBisector[] = VectorAlgebra.average(CACB, VectorAlgebra.scale(CBCG, -1));// Bisector
																						// of
																						// the
																						// HB1-CB-HB2
																						// angle
		double BPerpendicular[] = VectorAlgebra.cross(CACB, CBCG);// Perpendicular
																	// to the
																	// CA-CB-CG
																	// plane
		double GBisector[] = VectorAlgebra.average(CBCG, VectorAlgebra.scale(CGCD, -1));// Bisector
																						// of
																						// the
																						// HB1-CB-HB2
																						// angle
		double GPerpendicular[] = VectorAlgebra.cross(CBCG, CGCD);// Perpendicular
																	// to the
																	// CA-CB-CG
																	// plane
		double DBisector[] = VectorAlgebra.average(CGCD, VectorAlgebra.scale(CDN, -1));// Bisector
																						// of
																						// the
																						// HB1-CB-HB2
																						// angle
		double DPerpendicular[] = VectorAlgebra.cross(CGCD, CDN);// Perpendicular
																	// to the
																	// CA-CB-CG
																	// plane

		// We scale these vectors so they can be used as components of the C-H
		// bond vectors
		BBisector = VectorAlgebra.scale(BBisector,
				HBDist * (double) Math.cos(HBAngle / 2) / VectorAlgebra.norm(BBisector));
		GBisector = VectorAlgebra.scale(GBisector,
				HGDist * (double) Math.cos(HGAngle / 2) / VectorAlgebra.norm(GBisector));
		DBisector = VectorAlgebra.scale(DBisector,
				HDDist * (double) Math.cos(HDAngle / 2) / VectorAlgebra.norm(DBisector));
		BPerpendicular = VectorAlgebra.scale(BPerpendicular,
				HBDist * (double) Math.sin(HBAngle / 2) / VectorAlgebra.norm(BPerpendicular));
		GPerpendicular = VectorAlgebra.scale(GPerpendicular,
				HGDist * (double) Math.sin(HGAngle / 2) / VectorAlgebra.norm(GPerpendicular));
		DPerpendicular = VectorAlgebra.scale(DPerpendicular,
				HDDist * (double) Math.sin(HDAngle / 2) / VectorAlgebra.norm(DPerpendicular));

		// Calculate the hydrogen coordinates
		// Using the AMBER hydrogen-naming system (HB1, HB2 etc. as in the AA
		// templates)
