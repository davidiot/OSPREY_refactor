...
public class ResidueTypeDOF extends DegreeOfFreedom {
	// This degree of freedom is the residue type (e.g., AA type) at a
	// particular position
	...
	public void mutateTo(String resType) {
		// paramVal is the index in the ResidueTemplateLibrary of the new
		// parameter type
		...
		// 2x3 array of which atoms are used in the old and new residues to
		// align
		// to do the mutation
		...
		// we now construct a rigid-body motion that will map the sidechain (or
		// generally,
		// the non-"backbone" part, if not a standard amino acid) from the
		// template to the residue's
		...
		// if the sidechain attaches only to the first mutAlignAtom, this method
		// keeps bond lengths
		// exactly as in the template for sidechain, and as in the old backbone
		// otherwise
	...
	public String getCurResType() {// current residue type
		return res.fullName.substring(0, 3);// this is always the first three
	...
	public void apply(double paramVal) {
		throw new IllegalArgumentException(
				"ERROR: ResidueTypeDOF takes an residue type name" + " as argument; can't take " + paramVal);
	}
