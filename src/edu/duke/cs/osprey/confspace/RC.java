...
public class RC implements Serializable {
	...
	public ArrayList<Double> DOFmin;// minimum values of the DOFs for
									// conformations in the RC
	public ArrayList<Double> DOFmax;
	// note: for AA type we do not use DOFmin or DOFmax (can leave at 0 or
	// whatever): use AAType instead

	int RCIndex;// index within the RCs for this residue in the
				// PositionConfSpace

	public RC(String AAType, int rotNum, ArrayList<DegreeOfFreedom> DOFs, ArrayList<Double> DOFmin,
			ArrayList<Double> DOFmax, int RCIndex) {
		this.AAType = AAType;
		this.rotNum = rotNum;
		this.DOFs = DOFs;
		this.DOFmin = DOFmin;
	...
	public boolean isParametricallyIncompatibleWith(RC rc2) {
		// Two RCs are parametrically incompatible if and only if there is a DOF
		// that they share
		// for which they have different intervals
		// Thus, a conformation has a well-defined voxel if and only if it
		// contains no parametrically
