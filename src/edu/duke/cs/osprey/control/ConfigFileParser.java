...
public class ConfigFileParser {
	// An object that parses configuration files and uses them to initialize
	// objects needed
	// in various calculations (i.e. for data file loading, conf space
	// definition, pruning, etc.)
	...
	DEEPerSettings setupDEEPer() {
		// Set up the DEEPerSettings object, including the PertSet (describes
		// the perturbations)
		String runName = params.getValue("runName");

		DEEPerSettings dset = new DEEPerSettings(params.getBool("doPerturbations", false),
				params.getValue("perturbationFile", runName + ".pert"), params.getBool("selectPerturbations", true),
				params.getValue("startingPerturbationFile", "none"), params.getBool("onlyStartingPerturbations", false),
				params.getDouble("maxShearParam", 2.5), params.getDouble("maxBackrubParam", 2.5),
				params.getBool("selectLCAs", false), getFlexRes(), params.getValue("PDBNAME"));
	...
	private ArrayList<String[]> freeBBZoneTermini() {
		...
		for (String rt : params.searchParams("BBFREEBLOCK")) {
			// So for example BBFREEBLOCK0 120 125 would mean make a BBFreeBlock
			// for res 120-125
			// lexical ordering for blocks is OK
			String strandLimitsString = params.getValue(rt);

			String[] termini = { StringParsing.getToken(strandLimitsString, 1),
	...
	private ArrayList<String[]> moveableStrandTermini() {
		...
		// Let's say they can do this regardless of what doMinimize says (that's
		// for sidechains)
		ArrayList<String[]> ans = new ArrayList<>();

		for (String rt : params.searchParams("STRANDROTTRANS")) {
			if (params.getBool(rt, false)) {
				// So rt = STRANDROTTRANS0 here means strand 0 should translate
				// & rotate
	...
	SearchProblem getSearchProblem() {// this version is for a single search
										// problem...can modify for
		...
		return new SearchProblem(name, params.getValue("PDBNAME"), flexRes, allowedAAs, params.getBool("AddWT", true),
				params.getBool("doMinimize", false), params.getBool("UseEPIC", false), new EPICSettings(params),
				params.getBool("UseTupExp", false), dset, moveableStrands, freeBBZones,
				params.getBool("useEllipses", false), params.getBool("useERef", false));
	}

	ArrayList<String> getFlexRes() {
		// list of flexible residues. PDB-based residue numbers
		// we'll include all flexible residues: for compatibility (MAY BE
		// TEMPORARY),
		...
		int numStrands = params.searchParams("STRANDMUT").size() - params.searchParams("STRANDMUTNUMS").size();

		// must go through the strands in order to get the right residues
		for (int smNum = 0; smNum < numStrands; smNum++) {// must do these in
															// order
	...
	ArrayList<ArrayList<String>> getAllowedAAs() {
		...
		for (int str = 0; true; str++) {
			...
			if (numRecordsInStrand == 0) // finished with strands that have
											// flexible residues
				break;
		}
	...
	PruningControl setupPruning(SearchProblem searchSpace, double pruningInterval, boolean useEPIC, boolean useTupExp) {
		// setup pruning. Conformations in searchSpace more than (Ew+Ival) over
		// the GMEC are liable to pruning

		// initialize the pruning matrix for searchSpace, if not already
		// initialized
		// or if pruningInterval lower (so it may have pruned tuples that
		// shouldn't
		...
		return new PruningControl(searchSpace, pruningInterval, params.getBool("TYPEDEP", false),
				params.getDouble("BOUNDSTHRESH", 100), params.getInt("ALGOPTION", 1), params.getBool("USEFLAGS", true),
				params.getBool("USETRIPLES", false), false, useEPIC, useTupExp, params.getDouble("STERICTHRESH", 100));// FOR
																														// NOW
																														// NO
																														// DACS
	}
	...
	// residue templates, rotamer libraries, forcefield parameters, and
	// Ramachandran data
	void loadData() {
		...
		// general forcefield data loaded into the ForcefieldParams in
		// EnvironmentVars
		ForcefieldParams curForcefieldParams = new ForcefieldParams(params.getValue("Forcefield", "AMBER"),
				params.getBool("DISTDEPDIELECT", true), params.getDouble("DIELECTCONST", 6.0),
				params.getDouble("VDWMULT", 0.95), useEEF1, // Only EEF1
															// solvation is part
															// of the forcefield
															// (P-B calls
															// Delphi)
				params.getDouble("SOLVSCALE", 0.5), params.getBool("HELECT", true), params.getBool("HVDW", true));

		EnvironmentVars.curEFcnGenerator = new EnergyFunctionGenerator(curForcefieldParams,
				params.getDouble("SHELLDISTCUTOFF", Double.POSITIVE_INFINITY), usePoissonBoltzmann);
		...
		// load template coordinates (necessary for all residues we might want
		// to mutate to)
		...
		// load rotamer libraries; the names of residues as they appear in the
		 ...
		 * 
		 * hElect = (new Boolean((String)rParams.getValue("HELECT",
		 * "true"))).booleanValue(); hVDW = (new
		 * Boolean((String)rParams.getValue("HVDW", "true"))).booleanValue();
		 * hSteric = (new
		 * Boolean((String)rParams.getValue("HSTERIC","false"))).booleanValue();
		 * distDepDielect = (new
		 * Boolean((String)rParams.getValue("DISTDEPDIELECT","true"))).
		 * booleanValue(); dielectConst = (new
		 * Double((String)rParams.getValue("DIELECTCONST","6.0"))).doubleValue()
		 * ; doDihedE = (new
		 * Boolean((String)rParams.getValue("DODIHEDE","false"))).booleanValue()
		 * ; doSolvationE = (new
		 * Boolean((String)rParams.getValue("DOSOLVATIONE","true"))).
		 * booleanValue(); solvScale = (new
		 * Double((String)rParams.getValue("SOLVSCALE","0.5"))).doubleValue();
		 * softvdwMultiplier = (new
		 * Double((String)rParams.getValue("VDWMULT","0.95"))).doubleValue();
		 * stericThresh = (new
		 * Double((String)rParams.getValue("STERICTHRESH","0.4"))).doubleValue()
		 * ; softStericThresh = (new
		 * Double((String)rParams.getValue("SOFTSTERICTHRESH","1.5"))).
		 * doubleValue();
		 * EnvironmentVars.setDataDir(rParams.getValue("DATADIR","./"));
		 * EnvironmentVars.setForcefld(rParams.getValue("FORCEFIELD","AMBER"));
		 * double entropyScale = (new
		 * Double((String)rParams.getValue("ENTROPYSCALE","0.0"))).doubleValue()
		 ...
		 * 
		 * EnvironmentVars.autoFix = new
		 * Boolean((String)rParams.getValue("AUTOFIX","true")).booleanValue();
		 * 
		 * String ramaGlyFile =
		 * (String)rParams.getValue("RAMAGLYFILE","rama500-gly-sym.data");
		 * 
		 * if( ! ramaGlyFile.equalsIgnoreCase("none") ){ String ramaFiles[] = {
		 * EnvironmentVars.dataDir + ramaGlyFile, EnvironmentVars.dataDir +
		 * (String)rParams.getValue("RAMAPROFILE","rama500-pro.data"),
		 * EnvironmentVars.dataDir +
		 * (String)rParams.getValue("RAMAGENFILE","rama500-general.data"),
		 * EnvironmentVars.dataDir +
		 * (String)rParams.getValue("RAMAPREPROFILE","rama500-prepro.data") };
		 * RamachandranChecker.getInstance().readInputFiles( ramaFiles ); }
		 * 
		 */
	}
