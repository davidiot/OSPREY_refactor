...
public class EnergyFunctionGenerator {
	// This is an object that generates an energy function (maps
	// conformation-->energy) for a molecule or a portion thereof
	// it specifies settings for how this energy should be estimated

	// Start with AMBER/EEF1, add Poisson-Boltzmann (instead of EEF1), and
	// ultimately QM and explicit water hopefully

	public ForcefieldParams ffParams;

	public double distCutoff;// distance cutoff for interactions (angstroms)

	public boolean usePoissonBoltzmann;// Use Poisson-Boltzmann energies for
										// solvation
	// Note: these will not be included in the single-res and pair energy
	// functions,
	// so we need to do only full-conf energies to get the Poisson-Boltzmann
	// term
	...
	public EnergyFunction resPairEnergy(Residue res1, Residue res2) {
		// make an energy function estimating the interaction energy between two
		// residues
	...
	public EnergyFunction intraAndShellEnergy(Residue res, ArrayList<Residue> shellResidues) {
		// internal energy of res, plus its interactions with the residues in
		// shellRes
	...
	public EnergyFunction fullConfEnergy(ConfSpace cSpace, ArrayList<Residue> shellResidues) {
		// make an energy function estimating the energy of all flexible
		// residues in cSpace,
		// plus any interactions with other (shell) residues that are within
		// distCutoff of anything in cSpace
		// this can be used as the full energy for any conformation in cSpace

		ArrayList<Residue> flexibleResidues = new ArrayList<>();// array list
																// for these so
																// they stay in
																// order
		...
		// now we want the full energy (1-body + all interactions) of the
		// flexibleResidues,
		// plus the interactions of the flexibleResidues with the shellResidues
		// (We don't count interactions among the shellResidues because they
		// never change)
